## Makefile
## Summary: Cross-compilation builder for chat.
##
## Author:  KaisarCode
...
NATIVE_EXE_EXT :=
NATIVE_SHARED_NAME := libchat.so
NATIVE_IMPORT_LIBRARY :=

ifeq ($(NATIVE_PLATFORM),windows)
NATIVE_EXE_EXT := .exe
NATIVE_SHARED_NAME := libchat.dll
NATIVE_IMPORT_LIBRARY := -DCHAT_TEST_IMPORT_LIBRARY=$(CURDIR)/$(BIN_DIR)/$(NATIVE_TARGET)/libchat.dll.a
endif

ifeq ($(NATIVE_PLATFORM),macos)
NATIVE_SHARED_NAME := libchat.dylib
endif
...
define linux_target
	...
	@if [ ! -f $(BUILD_DIR)/$(subst /,-,$(1))-linux/CMakeCache.txt ]; then \
		cmake -S . -B $(BUILD_DIR)/$(subst /,-,$(1))-linux \
			...
			-DCHAT_BUILD_VERSION=$(BUILD_VERSION) \
			-DCMAKE_C_COMPILER=$(2) \
...
define windows_target
	...
	$(call cmake_build,$(BUILD_DIR)/$(1)-windows,cmake -S . -B $(BUILD_DIR)/$(1)-windows -DCMAKE_BUILD_TYPE=Release -DCMAKE_SYSTEM_NAME=Windows -DCHAT_BUILD_VERSION=$$ver -DCMAKE_C_COMPILER=$(2) -DCMAKE_RUNTIME_OUTPUT_DIRECTORY=$(CURDIR)/$(BUILD_DIR)/$(1)-windows/out -DCMAKE_ARCHIVE_OUTPUT_DIRECTORY=$(CURDIR)/$(BIN_DIR)/$(1)/windows -DCMAKE_LIBRARY_OUTPUT_DIRECTORY=$(CURDIR)/$(BIN_DIR)/$(1)/windows -G Ninja -Wno-dev > /dev/null)
	@cp $(BUILD_DIR)/$(1)-windows/out/chat.exe $(BIN_DIR)/$(1)/windows/chat.exe
	@cp $(BUILD_DIR)/$(1)-windows/out/libchat.dll $(BIN_DIR)/$(1)/windows/libchat.dll
	@echo "OK $(1)/windows"
endef
...
test:
	@if [ -n "$(filter wine,$(MAKECMDGOALS))" ]; then \
		...
		if [ ! -f $(BIN_DIR)/x86_64/windows/libchat.dll ] || [ ! -f $(BIN_DIR)/x86_64/windows/libchat.dll.a ] || [ ! -f $(BIN_DIR)/x86_64/windows/chat.exe ]; then \
		...
		if [ ! -f $(BUILD_DIR)/test-wine/CMakeCache.txt ]; then \
			cmake -S . -B $(BUILD_DIR)/test-wine \
				...
				-DCHAT_BUILD_VERSION=$(BUILD_VERSION) \
				-DCHAT_TEST_SHARED_LIBRARY=$(CURDIR)/$(BIN_DIR)/x86_64/windows/libchat.dll \
				-DCHAT_TEST_IMPORT_LIBRARY=$(CURDIR)/$(BIN_DIR)/x86_64/windows/libchat.dll.a \
				-DCHAT_TEST_CLI=$(CURDIR)/$(BIN_DIR)/x86_64/windows/chat.exe \
				-DCMAKE_CROSSCOMPILING_EMULATOR=$(WINE) \
				-G Ninja -Wno-dev > /dev/null; \
		fi; \
		cmake --build $(BUILD_DIR)/test-wine --target chat_contract_test; \
		...
		if [ ! -f $(BIN_DIR)/$(NATIVE_TARGET)/$(NATIVE_SHARED_NAME) ] || [ ! -f $(BIN_DIR)/$(NATIVE_TARGET)/chat$(NATIVE_EXE_EXT) ]; then \
		...
		if [ ! -f $(BUILD_DIR)/test/CMakeCache.txt ]; then \
			cmake -S . -B $(BUILD_DIR)/test \
				...
				-DCHAT_BUILD_TESTS=ON \
				-DCHAT_BUILD_VERSION=$(BUILD_VERSION) \
				-DCHAT_TEST_SHARED_LIBRARY=$(CURDIR)/$(BIN_DIR)/$(NATIVE_TARGET)/$(NATIVE_SHARED_NAME) \
				-DCHAT_TEST_CLI=$(CURDIR)/$(BIN_DIR)/$(NATIVE_TARGET)/chat$(NATIVE_EXE_EXT) \
				$(NATIVE_IMPORT_LIBRARY) \
				-G Ninja -Wno-dev > /dev/null; \
		fi; \
		cmake --build $(BUILD_DIR)/test --target chat_contract_test; \
